...
function convertToSupabase(data: Omit<Requisition, 'id' | 'createdAt' | 'updatedAt'>) {
  ...
  return {
    ...
    invoice_value: typeof data.invoiceValue === 'number' ? data.invoiceValue : (typeof data.invoiceValue === 'string' && data.invoiceValue !== '' ? parseFloat(data.invoiceValue.replace(',', '.')) || 0 : 0),
    ...
    freight_value: typeof data.freightValue === 'number' ? data.freightValue : (typeof data.freightValue === 'string' && data.freightValue !== '' ? parseFloat(data.freightValue.replace(',', '.')) || 0 : 0),
