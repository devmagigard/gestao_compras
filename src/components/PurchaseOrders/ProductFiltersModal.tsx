...
import { PurchaseOrderFilterState } from '../../types';
import { PURCHASE_ORDER_STATUSES, CURRENCIES } from '../../utils/constants';

interface ProductFiltersModalProps {
  isOpen: boolean;
  onClose: () => void;
  currentFilters: PurchaseOrderFilterState;
  onApplyFilters: (filters: PurchaseOrderFilterState) => void;
  uniqueValues: {
...
export function ProductFiltersModal({ 
  ...
  const [localFilters, setLocalFilters] = useState<PurchaseOrderFilterState>(currentFilters);
  ...
  const handleChange = (field: keyof PurchaseOrderFilterState, value: any) => {
    setLocalFilters(prev => ({ ...prev, [field]: value }));
  };

  // Contar filtros ativos (excluindo campos de busca que ficam sempre visíveis)
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select
                value={localFilters.currencyFilter}
                onChange={(e) => handleChange('currencyFilter', e.target.value)}
                className={`w-full px-4 py-3 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 cursor-pointer ${
                  isDarkMode 
                    ? 'bg-gray-700 border-gray-600 text-white' 
                    : 'bg-white border-gray-300 text-gray-900'
          ...
          {/* Filtro de Entrega - Largura Total */}
          <div className="space-y-2">
            <label className={`block text-sm font-medium ${
              isDarkMode ? 'text-gray-300' : 'text-gray-700'
            }`}>
            ...
            <select
              value={localFilters.deliveryFilter}
              onChange={(e) => handleChange('deliveryFilter', e.target.value)}
              className={`w-full px-4 py-3 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 cursor-pointer ${
                isDarkMode 
                  ? 'bg-gray-700 border-gray-600 text-white' 
                  : 'bg-white border-gray-300 text-gray-900'
          ...
          {activeFiltersCount > 0 && (
            <div className={`p-4 rounded-lg border-l-4 ${
              ...
              <div className="space-y-1 text-sm">
                ...
                {localFilters.currencyFilter !== 'all' && (
                  <div>• Moeda: {localFilters.currencyFilter}</div>
                )}
                {localFilters.deliveryFilter !== 'all' && (
                  <div>• Entrega: {
                    localFilters.deliveryFilter === 'delayed' ? 'Atrasadas' :
                    localFilters.deliveryFilter === 'upcoming' ? 'Próximas' :
                    'Precisam Atenção'
                  }</div>
                )}
              </div>
